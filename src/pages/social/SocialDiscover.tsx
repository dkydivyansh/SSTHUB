...
import { Search, Filter, X, Mail, User, ShieldAlert, Award } from 'lucide-react';
import { Link } from 'react-router-dom';

export default function SocialDiscover() {
  const [query, setQuery] = useState('');
  const [filters, setFilters] = useState({ batch: '', group: '', type: '' });
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/social_discover.php?${queryParams.toString()}`);
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full bg-[#f4f4f5] border-4 border-black p-4 pl-12 font-bold outline-none focus:border-[#3B82F6] transition-colors text-lg"
              />
            </div>
          </div>
          
          {/* Filters */}
          <div className="flex flex-wrap gap-4 items-center border-t-4 border-black pt-4">
            <Filter size={20} className="text-black/50" />
            <select
              value={filters.batch}
              onChange={(e) => setFilters({ ...filters, batch: e.target.value })}
              className="bg-white border-4 border-black p-2 font-black uppercase tracking-widest text-sm focus:border-[#3B82F6] outline-none cursor-pointer"
            >
            ...
            <div className="flex flex-col items-center text-center mt-4">
              ...
              {(selectedUser.type === 'faculty' || selectedUser.type === 'admin') && (
                <div className="bg-red-500 text-white font-black uppercase tracking-widest text-xs px-3 py-1 border-2 border-black mb-2 flex items-center gap-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  <ShieldAlert size={14} /> {selectedUser.type}
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-2 w-full">
                <button 
                  disabled
                  className="flex flex-col items-center gap-2 p-3 bg-white border-4 border-black opacity-50 cursor-not-allowed shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
                >
                  <Mail size={20} />
