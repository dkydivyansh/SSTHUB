...
try {
    ...
    if (!$is_admin) {
        http_response_code(403);
        echo json_encode(['status' => 'error', 'message' => 'Only group admins can pin posts']);
        exit();
    }
    ...
    // Update the post
    $stmt = $conn->prepare("UPDATE $table SET pinned = ? WHERE id = ? AND groupid = ?");
    $stmt->execute([$pinned ? 1 : 0, $post_id, $group_id]);

    echo json_encode([
        'status' => 'success',
        'message' => ucfirst($post_type) . ($pinned ? ' pinned' : ' unpinned') . ' successfully'
    ]);
